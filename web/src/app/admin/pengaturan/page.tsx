...
export default function AdminPengaturanPage() {
  ...
  const [deleteItem, setDeleteItem] = useState<ConfigSla | null>(null);
  ...
  return (
    <div className="space-y-[25px]">
      ...
      <div className="bg-white dark:bg-[#0c1427] p-5 rounded-xl border border-gray-100 dark:border-[#172036]">{query.isLoading ? <div className="animate-pulse h-48 bg-gray-200 dark:bg-[#172036] rounded-md" /> : <div className="table-responsive overflow-x-auto"><table className="w-full"><thead><tr>{["Jabatan", "Jenis Layanan", "SLA Hari", "SLA Jam", "Eskalasi Hari", "Aksi"].map((h) => <th className="font-medium text-left px-[20px] py-[11px] bg-primary-50 dark:bg-[#15203c]" key={h}>{h}</th>)}</tr></thead><tbody>{(query.data ?? []).map((item) => <tr key={item.id}><td className="px-[20px] py-[15px] border-b">{jabatanLabels[item.jabatan] ?? item.jabatan}</td><td className="px-[20px] py-[15px] border-b">{item.jenisLayanan?.nama ?? "Semua Jenis"}</td><td className="px-[20px] py-[15px] border-b">{item.slaHari}</td><td className="px-[20px] py-[15px] border-b">{item.slaJam}</td><td className="px-[20px] py-[15px] border-b">{item.eskalasiHari ?? "-"}</td><td className="px-[20px] py-[15px] border-b"><button className="text-primary-500 mr-3" type="button" onClick={() => open(item)}>Edit</button><button className="text-danger-500" type="button" onClick={() => setDeleteItem(item)}>Hapus</button></td></tr>)}</tbody></table></div>}</div>
      ...
      <ConfirmModal
        ...
        description={`Hapus konfigurasi SLA untuk ${deleteItem ? jabatanLabels[deleteItem.jabatan] ?? deleteItem.jabatan : "tahap ini"}? Perubahan ini memengaruhi perhitungan SLA berikutnya.`}
        ...
        onConfirm={() => {
          ...
          actions.remove.mutate(deleteItem.id, {
            onSuccess: () => {
              setDeleteItem(null);
              showToast("Config SLA berhasil dihapus", "success");
            },
          });
        }}
        showTextarea={false}
