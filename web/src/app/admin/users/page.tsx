    ...
    <div className="space-y-[25px]">
      ...
      <div className="trezo-card bg-white dark:bg-[#0c1427] p-[20px] md:p-[25px] rounded-md">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <select className="h-[45px] rounded-md border border-gray-200 dark:border-[#172036] bg-white dark:bg-[#0c1427] px-[14px]" value={roleId} onChange={(event) => { setRoleId(event.target.value); setPage(1); }}><option value="">Semua Role</option>{(roles.data ?? []).map((role) => <option key={role.id} value={role.id}>{displayRoleLabel(role.nama)}</option>)}</select>
