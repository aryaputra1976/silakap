...
export default function BuatAsnPage() {
  ...
  useEffect(() => {
    if (user?.roleNama && user.roleNama !== "Admin_Sistem") {
      router.replace("/asn");
    }
  }, [router, user]);

  const onSubmit = (values: AsnFormValues) => {
    createAsn.mutate(toPayload(values), {
      onSuccess: (response) => {
        showToast("Data ASN berhasil dibuat", "success");
        const id = response.data.data.id;
        window.setTimeout(() => router.push(`/asn/${id}`), 700);
      },
      onError: () => showToast("Gagal membuat data ASN", "error"),
    });
  };
    ...
    <div className="space-y-[25px]">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="!mb-0">Tambah ASN</h1>
        </div>
        <Link
          href="/asn"
      ...
      >
        ...
        {activeTab === "Data Pribadi" ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-[20px]">
            ...
            <label>Nama<input className={inputClass} {...register("nama")} />{errors.nama ? <span className="text-danger-500 text-sm">{errors.nama.message}</span> : null}</label>
        ...
        <div className="mt-[25px] flex flex-wrap items-center gap-3">
          ...
          >
            {createAsn.isPending ? "Menyimpan..." : "Simpan"}
          </button>
          <Link href="/asn" className="py-[10px] px-[20px] rounded-md border border-gray-200 dark:border-[#172036]">
            Batal
          </Link>
        </div>
      </form>
